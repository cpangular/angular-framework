...
export const accessibleStyleSheets = from(document.styleSheets).where((ss) => {
  try {
    return !!ss.cssRules;
  } catch {
    return false;
...
export function findThemeDefinitionRule(
  type: string,
  name: string
): CSSStyleRule | undefined {
  return definitionRules.firstOrDefault(
    (_) =>
      _.style.getPropertyValue(define.propertyName(type)).trim() === name.trim()
  );
}
