...
@Directive()
export abstract class ApplicationLayout
  implements IApplicationLayoutComponent, OnDestroy
{
  private __subs: Subscription = new Subscription();

  private breakpointsResolve: BreakpointResolver<string[]> =
    this.breakpointService.createResolver([
  ...
  private orientationResolve: BreakpointResolver<string[]> =
    this.breakpointService.createResolver([
      ['landscape', ['landscape']],
      ['portrait', ['portrait']],
    ]);
  ...
  constructor(
    ...
    this.__subs.add(
      this.breakpointsResolve.subscribe((v) => {
        const bp = v[0];
    ...
    this.__subs.add(
      this.orientationResolve.subscribe((v) => {
