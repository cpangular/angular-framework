...
class ZoneAwareBreakpointObservable extends Observable<number> implements BreakpointObservable {
...
export class BreakpointService {
  ...
  public createObservable(
    ...
    if (typeof bp === 'string') {
      const dict = Object.assign({}, DEFAULT_BREAKPOINTS, this.defaultBreakpoints ?? {}, opts?.breakpointDict ?? {});
      if (!dict[bp]) {
        throw new Error(`No breakpoint with name "${bp}" defined.`)
      }
      return new ZoneAwareBreakpointObservable(this.zone, dict[bp], opts?.element);
    }
  ...
  public createResolver<T>(
    ...
    const dict = Object.assign({}, DEFAULT_BREAKPOINTS, this.defaultBreakpoints ?? {}, opts?.breakpointDict ?? {});
    return new ZoneAwareBreakpointResolver<T>(this.zone, conditions, dict, opts?.element) as BreakpointResolver<T>;
  }
