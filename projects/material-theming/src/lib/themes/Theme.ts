...
import { ThemeBackgroundRefChange, ThemeCssChange, ThemeForegroundRefChange, ThemePaletteRefChange } from '../events/ThemeEvents';
...
export class Theme extends ThemeDefinitionBase {
  ...
  protected createCssChangeEvent(newValue: string, oldvalue: string): ThemingChangeEvent<Theme, string> {
    return new ThemeCssChange(this, oldvalue, newValue);
  }

  public get themeStyles(): IEnumerable<string> {
    return from(this.rule.style).where(_ => _.startsWith(`--theme-${this.name}-`));
  }
  ...
  public get paletteRefs(): IThemePaletteRef[] {
    const props = from(this.paletteKeys)
      .selectMany(palette =>
        this.themeStyles
          .where(s => s.startsWith(themeColor.propertyName(this.name, palette, '')))
          .select(property => ({
            property: property.split(themeColor.propertyName(this.name, palette, ''))[1],
            palette
          }))
      ).groupBy(_ => _.palette, _ => _, (k, g) => ({
        name: k,
        variants: g.toObject(
          _ => _.property,
          _ => this.rule.style.getPropertyValue(themeColor.propertyName(this.name, _.palette, _.property))?.trim()
        )
      })).select(_ => {
  ...
  public setPaletteRef(ref: IThemePaletteRef): void {
    ...
    this.rule.style.setProperty(themeColor.propertyName(this.name, ref.name, 'name'), ref.palette);
    for (const key in ref.variants) {
      if (Object.prototype.hasOwnProperty.call(ref.variants, key)) {
        const value = ref.variants[key];
        this.rule.style.setProperty(themeColor.propertyName(this.name, ref.name, key), value);
      }
    }
    this.emitChange(new ThemePaletteRefChange(this, old, this.getPaletteRef(ref.name)));
  ...
  public removePaletteRef(name: string): void {
    ...
    if (old) {
      const olddCss = this.toString();
      this.themeStyles
        .where(_ => _.startsWith(themeColor.propertyName(this.name, name, '')))
        .forEach(_ => {
  ...
  public getBackgroundRef(): IThemePaletteRef {
    ...
    return {
      name: 'background',
      palette: this.rule.style.getPropertyValue(themeBg.propertyName(this.name, 'name')).trim(),
      variants: this.themeStyles
        .where(_ => _.startsWith(bgProperty) && !_.endsWith('-name'))
        .select(_ => _.split(bgProperty)[1])
        .toObject(_ => _, _ => this.rule.style.getPropertyValue(themeBg.propertyName(this.name, _)).trim()) as any
    };
  }
  ...
  public setBackgroundRef(ref: IThemePaletteRef): void {
    ...
    const olddCss = this.toString();
    this.rule.style.setProperty(themeBg.propertyName(this.name, 'name'), ref.palette);
    for (const key in ref.variants) {
      if (Object.prototype.hasOwnProperty.call(ref.variants, key)) {
        const value = ref.variants[key];
        this.rule.style.setProperty(themeBg.propertyName(this.name, key), value);
      }
    }
    this.emitChange(new ThemeBackgroundRefChange(this, old, this.getBackgroundRef()));
    this.emitChange(new ThemeCssChange(this, olddCss, this.toString()));
  }
  ...
  public getForegroundRef(): IThemePaletteRef {
    ...
    return {
      name: 'foreground',
      palette: this.rule.style.getPropertyValue(themeFg.propertyName(this.name, 'name')).trim(),
      variants: this.themeStyles
        .where(_ => _.startsWith(bgProperty) && !_.endsWith('-name'))
        .select(_ => _.split(bgProperty)[1])
        .toObject(_ => _, _ => this.rule.style.getPropertyValue(themeFg.propertyName(this.name, _)).trim()) as any
    };
  }
  ...
  public setForegroundRef(ref: IThemePaletteRef): void {
    ...
    const olddCss = this.toString();
    this.rule.style.setProperty(themeFg.propertyName(this.name, 'name'), ref.palette);
    for (const key in ref.variants) {
      if (Object.prototype.hasOwnProperty.call(ref.variants, key)) {
        const value = ref.variants[key];
        this.rule.style.setProperty(themeFg.propertyName(this.name, key), value);
      }
    }
    this.emitChange(new ThemeForegroundRefChange(this, old, this.getForegroundRef()));
    this.emitChange(new ThemeCssChange(this, olddCss, this.toString()));
  }
